...
- Bundled macOS release artifacts now regenerate a root `NOTICES` file and ship it in every tarball; release remains blocked behind `ALLOW_BUNDLED_TOOL_REDISTRIBUTION=true`, and the bundle scope is still `scdl` + `yt-dlp` invoked via subprocess only.
